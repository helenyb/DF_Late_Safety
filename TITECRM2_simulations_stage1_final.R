##TITECRM2 (2 parameter logistic model, real doses)
#simulations
#STAGE 1
#packages
library(doParallel)
...
for(sc in 1:length(scen_list)){
  ...
  cal_time <- system.time({
    assign(paste(c("TITECRM2_simulation_s",sc),collapse = ""), foreach(i=1:nsims, combine = list) %dopar% {
      ##function
      TITECRM_2parlog_stage2(seed=i,tru_cyc1=scen_list[[sc]],co_size=co_size,ncohorts=ncohorts,target=target,doses=doses,p_a_mean=p_a_mean,p_b_mean=p_b_mean,
                             p_a_prec=p_a_prec,p_b_prec=p_b_prec,nTTP_array=nTTP_array,ncycles=ncycles,cycle_dec=cycle_dec,
                             hard.safety.rule = 0, safety.stopping.low.unsafe= F, safety.stopping.high.toosafe = F,precision.stopping = F)
      
    })
  })
  save.image("TITECRM2_simulations_stage1full.RData")
}
