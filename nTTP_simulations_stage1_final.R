...
target<-0.289 #target nTTP for cycle 1
ncycles<-3
cycle_dec<-1/3
nsims<-5000
ndoses<-length(doses)
...
nTTP_prior_par<-c(doseparmean, doseparvar,intparmean,intparvar,
                  cycleparmean, cycleparvar)
...
for(sc in 1:length(scen_list)){
  ...
  cal_time <- system.time({
    assign(paste(c("nTTP_simulation_s",sc),collapse = ""), foreach(i=1:nsims, combine = list) %dopar% {
      ##function
      nTTP_stage2(seed=i,tru_cyc1=scen_list[[sc]],co_size=co_size,ncohorts=ncohorts,target=target,doses=doses,nTTP_array=nTTP_array,ncycles=ncycles,prior_vec=nTTP_prior_par,cycle_dec=cycle_dec,
                  hard.safety.rule = 0, safety.stopping.low.unsafe= F, safety.stopping.high.toosafe = F,precision.stopping = F)
      
    })
  })
  save.image("nTTP_simulations_stage1full.RData")
}
