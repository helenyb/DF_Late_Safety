...
##pre-amble functions
#for normal prior
TITE_CRM_Post_Bayes_Np<-function(beta,weights_obs,DLT_obs,dose_obs,doses_vec,prior_mean,prior_var){
...
#posterior density for beta FOR ALL CYCLES 
post_betaALL<-function(beta,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                       dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
  ncon<-integrate(TITE_CRM_Post_Bayes_vector_Np,-10,10,weights_obs=weights_obs,DLT_obs=DLT_obs,
                  dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)$value
  
  TITE_CRM_Post_Bayes_vector_Np(beta=beta,weights_obs=weights_obs,DLT_obs=DLT_obs,
                                dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)/ncon
  
}


#posterior density for beta FOR CYCLE 1
post_beta<-function(beta,weights_obs=current.patient.data_cyc1$patient_weights,DLT_obs=current.patient.data_cyc1$current.DLT,
                    dose_obs=current.patient.data_cyc1$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
  ncon<-integrate(TITE_CRM_Post_Bayes_vector_Np,-10,10,weights_obs=weights_obs,DLT_obs=DLT_obs,
                  dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)$value
...
#cdf of beta. value offset by y 
solvfunc<-function(c,y,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                   dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
  
  integrate(post_betaALL,-10,c,weights_obs=weights_obs,DLT_obs=DLT_obs,
            dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)$value-y
...
#cdf of beta
beta_cdf<-function(c,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                   dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
...
#cdf of MTD
MTD_cdf<-function(c,target,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                  dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
  if(c<=0){
    return(0)
  }else if(c>=1){
    ...
    return(beta_cdf(log(log(target)/log(c)),weights_obs=weights_obs,DLT_obs=DLT_obs,
                    dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var))
    
  }
}
#cdf of MTD with out put offset by y
MTD_cdf_offset<-function(c,y,target,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                         dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
  beta_cdf(log(log(target)/log(c)),weights_obs=weights_obs,DLT_obs=DLT_obs,
           dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)-y
}
#cdf of abs(MTD-med(MTD))
MTD_med_abs_cdf<-function(c,target,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                          dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
  medMTD<-uniroot(MTD_cdf_offset,c(1e-5,1-1e-5),y=0.5,target=target,weights_obs=weights_obs,DLT_obs=DLT_obs,
                  dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)$root
  return(MTD_cdf(c+medMTD,target=target,weights_obs=weights_obs,DLT_obs=DLT_obs,
                 dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)-
           MTD_cdf(-c+medMTD,target=target,weights_obs=weights_obs,DLT_obs=DLT_obs,
                   dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var))
  
}
#cdf of abs(MTD-med(MTD)) with out put offset by y
MTD_med_abs_cdf_offset<-function(c,y,target,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                                 dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
  MTD_med_abs_cdf(c=c,target=target,weights_obs=weights_obs,DLT_obs=DLT_obs,
                  dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)-y
}

#Mat function

mat_titecrm<-function(target,constant = 1.4826,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                      dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var){
  
  constant*uniroot(MTD_med_abs_cdf_offset,c(1e-5,1-1e-5),y=0.5,target=target,weights_obs=weights_obs,DLT_obs=DLT_obs,
                   dose_obs=dose_obs,doses_vec=doses_vec,prior_mean=prior_mean,prior_var=prior_var)$root
...
TITECRM_stage2<-function(seed,tru_cyc1,co_size,ncohorts ,target,doses,prior_mean,prior_var,nTTP_array,ncycles,cycle_dec,
                         sufficient.information=T,hard.safety.rule=95,safety.stopping.low.unsafe=T,safety.stopping.high.toosafe=T,
                         kfold.skipping=T,kfold=2,precision.stopping=T,initial.one.cycle=T){
  set.seed(seed)
  ...
  while(stop==0){
    ...
    if(initial==0){
      #normalizing conastant
      ncon<-integrate(TITE_CRM_Post_Bayes_vector_Np,-10,10,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                      dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean,prior_var=prior_var)
      #posterior mean of beta parameter
      b_est<-integrate(int_Bayes_Np,-10,10,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                       dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean,prior_var=prior_var)$value/ncon$value
      nextdose<-dose_recom(betahat=b_est,doses,target=target)
      ...
      if((safety.stopping.high.toosafe==T)&(npats_doses[ndoses]>0)){
        ...
        #transform according to power model
        cycJ_0.3l<-integrate(post_beta,log(log(0.3)/log(doses[ndoses])),10,weights_obs=current.patient.data_cyc1$patient_weights,DLT_obs=current.patient.data_cyc1$current.DLT,
                             dose_obs=current.patient.data_cyc1$dose.level,doses_vec=doses,prior_mean,prior_var=prior_var)$val
      ...
      if((safety.stopping.low.unsafe==T)&(npats_doses[1]>0)){
        ...
        #transform according to power model
        cyc1_0.3g<-integrate(post_beta,-10,log(log(0.3)/log(doses[1])),weights_obs=current.patient.data_cyc1$patient_weights,DLT_obs=current.patient.data_cyc1$current.DLT,
                             dose_obs=current.patient.data_cyc1$dose.level,doses_vec=doses,prior_mean,prior_var=prior_var)$val
    ...
    #      break
     ...
     #    break
      ...
      if((precision.stopping==T)&(sum(npats_doses)>=9)){
        ...
        medMTD<-uniroot(MTD_cdf_offset,c(1e-5,1-1e-5),y=0.5,target=target,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                        dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var)$root
        
        CV<-mat_titecrm(target=target,constant = 1.4826,weights_obs=current.patient.data$patient_weights,DLT_obs=current.patient.data$current.DLT,
                        dose_obs=current.patient.data$dose.level,doses_vec=doses,prior_mean=prior_mean,prior_var=prior_var)/medMTD
    ...
    if(nrow(patient.data)==(ncohorts*co_size)){ #max patients reached
      ...
      dose_rec<-dose_recom(betahat=b_est,doses,target=target)
      stop<-3
      stop_vec[3]<-1
    }
