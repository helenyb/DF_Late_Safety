...
#function to create decision matrix
#output has col = number of patients that have been fully observed on current dose
#row = number of DLTs on current dose
#-1/0/1 for de-escalate/stay/escalate
mpti2_decision_matrix_function<-function(target, eps1,eps2,co_size,ncohorts){
  ...
  #define intervals by lower and upper
  decision<-matrix(NA,nrow=co_size*ncohorts+1,ncol=co_size*ncohorts)
  
  for(nj in 1:(co_size*ncohorts)){
    ...
    for(yj in 0:nj){
      ...
      probs_int<-sapply(c(1:length(class_int)),function(x) pbeta(uppers_int[x],1+yj,1+nj-yj)-pbeta(lowers_int[x],1+yj,1+nj-yj))
      ...
      decision[yj+1,nj]<-class_int[highest_upm]
...
R_TPI_stage2<-function(seed,tru_cyc1,co_size,ncohorts ,target,doses,eps1=0.05,eps2=0.05,C=3,klim=3,nTTP_array,ncycles,cycle_dec,
  ...
  decision_mat<-mpti2_decision_matrix_function(target=target, eps1=eps1,eps2=eps2,
                                               co_size=co_size,ncohorts=ncohorts)
  ...
  while(stop==0){
   # browser()
    ...
    ##run-in enrollment
    if(npats_doses[currentdosej]<C){
      #recruit on same dose (nextdose is the same)
    }else if((npats_doses[currentdosej]>=C)&(nj==0)){
      #suspend recruitment
      nextdose<-NA
    }else{ #rolling enrollment
      ...
      decisionj_mtpi2[current.time]<-decision_mat[yj+1,nj]
      
      if(mj>C){
        decision_Rmtpi[current.time]<-100
      } else if(mj==0){
        decision_Rmtpi[current.time]<-decisionj_mtpi2[current.time]
      }else{
        if(decisionj_mtpi2[current.time]==-1){
          if(decision_mat[yj+1,nj+mj]==-1){
            decision_Rmtpi[current.time]<-(-1)
          } else{
            decision_Rmtpi[current.time]<-0
          }
          
        }else if(decisionj_mtpi2[current.time]==0){
          if(decision_mat[yj+1,nj+mj]==0){
            decision_Rmtpi[current.time]<-0
          } else if (decision_mat[yj+1,nj+mj]==1){
            if(kj<klim){
              decision_Rmtpi[current.time]<-0
            }else{
              decision_Rmtpi[current.time]<-100 #suspend 
            }
            
            
          }
        }else if(decisionj_mtpi2[current.time]==1){
          if(decision_mat[yj+1+mj,nj+mj]==1){
            decision_Rmtpi[current.time]<-1
          } else if (kj<klim){
            decision_Rmtpi[current.time]<-0
          }else{
      ...
      if(decision_Rmtpi[current.time]==100){
        #suspend recruitment
        nextdose<-NA
      }else if(decision_Rmtpi[current.time]==-1){
        nextdose<-max(1,currentdosej-1)
      } else if(decision_Rmtpi[current.time]==0){
        nextdose<-currentdosej

      } else if(decision_Rmtpi[current.time]==1){
        nextdose<-min(length(doses),currentdosej+1)
      }
