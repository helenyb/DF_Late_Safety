...
##define grid for parameter values #new
var_val<-1

scaled_doses<-c(0.05,.1,.15,.2,.25,.3)
...
for(sc in 1:length(scen_list)){
  ...
  cal_time <- system.time({
    assign(paste(c("TITECRM_simulation_s",sc),collapse = ""), foreach(i=1:nsims, combine = list) %dopar% {
      ##function
      TITECRM_stage2(seed=i,tru_cyc1=scen_list[[sc]],co_size=co_size,ncohorts=ncohorts,target=target,doses=scaled_doses,prior_mean=0,prior_var=var_val,nTTP_array=nTTP_array,ncycles=ncycles,cycle_dec=cycle_dec,
                     hard.safety.rule = 0, safety.stopping.low.unsafe= F, safety.stopping.high.toosafe = F,precision.stopping = F)
      
    })
  })
  save.image("TITECRM_simulations_stage1full.RData")
}
