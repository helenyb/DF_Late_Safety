...
for(sc in 1:length(scen_list)){
  ...
  cal_time <- system.time({
    assign(paste(c("TITE_mTPI2_simulation_s",sc),collapse = ""), foreach(i=1:nsims, combine = list) %dopar% {
      ##function
      TITE_mTPI2_stage2(seed=i,tru_cyc1=scen_list[[sc]],co_size=co_size,ncohorts=ncohorts,
                   eps1 =eps1,  eps2=eps2, 
  ...
  save.image("TITE_mTPI2_simulations_stagefull.RData")
}
