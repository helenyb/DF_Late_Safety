...
#source code
source("TITE_mTPI2_function_stage2_comp-v5.R")
...
TITE_mTPI2_prior_par_list<-list()
...
for(p1 in 1: length(a_vec)){
  for(p2 in 1:length(b_vec)){
    ...
    TITE_mTPI2_prior_par_list[[index]]<-c(target-a_vec[p1]*target,
                                          b_vec[p2]*target-target)
...
for(pc in 1:length(TITE_mTPI2_prior_par_list)){
  ...
  for(sc in 1:length(prior_cal_scen_list)){
    ...
    cal_time <- system.time({
      assign(paste(c("TITE_mTPI2_priorcal_s",sc,"_p",pc),collapse = ""), foreach(i=1:1000, combine = list) %dopar% {
        ##function
        TITE_mTPI2_stage2(seed=i,tru_cyc1=prior_cal_scen_list[[sc]],co_size=co_size,ncohorts=ncohorts,
                          eps1 =TITE_mTPI2_prior_par_list[[pc]][1],  eps2=TITE_mTPI2_prior_par_list[[pc]][2], 
    ...
    save.image("TITE_mTPI2_priorcal_stage1.RData")
  }
}
