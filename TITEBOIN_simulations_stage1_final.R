...
for(sc in 1:length(scen_list)){
  ...
  cal_time <- system.time({
    assign(paste(c("TITEBOIN_simulation_s",sc),collapse = ""), foreach(i=1:nsims, combine = list) %dopar% {
      ##function
      TITEBOIN_stage2(seed=i,tru_cyc1=scen_list[[sc]],co_size=co_size,ncohorts=ncohorts,
                        a=aval,b=bval,alpha=alphaval,beta=betaval,
  ...
  save.image("TITEBOIN_simulations_stage1full.RData")
}
