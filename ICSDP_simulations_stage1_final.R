...
ICSDP_prior_r<-ICSDP_prior_func(pi1=pi1,piJ=piJ,num0=num0,ncycles=ncycles,cycle_dec=cycle_dec,ndoses=ndoses,rorq="r")

ICSDP_prior_q<-ICSDP_prior_func(pi1=pi1,piJ=piJ,num0=num0,ncycles=ncycles,cycle_dec=cycle_dec,ndoses=ndoses,rorq="q")
...
for(sc in 1:length(scen_list)){
  ...
  cal_time <- system.time({
    assign(paste(c("ICSDP_simulation_s",sc),collapse = ""), foreach(i=1:nsims, combine = list) %dopar% {
      ##function
      ICSDP_stage2(seed=i,tru_cyc1=scen_list[[sc]],co_size=co_size,ncohorts=ncohorts,target=target,doses=doses,rprior=ICSDP_prior_r,
                   qprior=ICSDP_prior_q,nTTP_array=nTTP_array,ncycles=ncycles,cycle_dec=cycle_dec,
                   hard.safety.rule = 0, safety.stopping.low.unsafe= F, safety.stopping.high.toosafe = F,precision.stopping = F)
      
    })
  })
  save.image("ICSDP_simulations_stage1full.RData")
}
